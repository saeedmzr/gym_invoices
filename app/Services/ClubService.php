...
class ClubService
{
    ...
    public function checkIn($club_id, User $user)
    {
        ...
        $invoice = $this->invoiceRepository->createInvoiceForAUserIfNotExists($club, $user);

        $invoice_lines = json_decode($invoice->invoice_lines);
        ...
        $invoice_lines[] = ['amount' => $cost_per_check_in, 'description' => $description];
